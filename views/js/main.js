...
var makeRandomPizza = function() {
  var pizza = "";
	function produceRandomNum(x){     //小小修改，用函数来产生随机数
		return Math.floor((Math.random() * x));
	}
  var numberOfMeats = produceRandomNum(4);//Math.floor((Math.random() * 4));
  var numberOfNonMeats = produceRandomNum(3);//Math.floor((Math.random() * 3));
  var numberOfCheeses = produceRandomNum(2);//Math.floor((Math.random() * 2));

  for (var i = 0; i < numberOfMeats; i++) {
    pizza += ingredientItemizer(selectRandomMeat());    //小小修改，改为+=
  }
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
  	var getPizzaSize=document.querySelector("#pizzaSize").innerHTML;   //小小修改，避免重复
    switch(size) {
      case "1":
        getPizzaSize = "Small";
        return;
  ...
  // 遍历披萨的元素并改变它们的宽度
  function changePizzaSizes(size) {   //第一个主要问题更改处
  	var newwidth;     //定义变量，但不赋值
  	switch(size){			//判断size值，用百分比调整值
  		case "1":
  			newwidth = 25;
  			break;
  		case "2":
...
function updatePositions() {
  ...
  window.performance.mark("mark_start_frame");
	var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));    //第二个主要问题更改处，把强制布局提出循环
